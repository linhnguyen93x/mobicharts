...
import Svg, { Circle, G, Line, Path, Rect, Text, TSpan } from 'react-native-svg'
import { svgPathProperties } from 'svg-path-properties'

import NativePath from './AnimatedSVG'
import NativeCircle from './AnimatedSVGCircle'
import { bottomAxisData, dummyData, leftAxisData } from './dummyData'
import createLegend from './utils/createLegend'
import { buildColorArray, calculateOverallLineChartData } from './utils/dataCalculation'
  ...
  treeManipulation () {
    ...
    circleInFirstLine = dataPointsVisible
      ? _.map (pointData, (d, i) => {
          ...
          return (
            <G key={i} delayPressIn={0}
              // onPressIn={() => this.setState({selected: d.x})}
              >
              {animation
                ? <NativeCircle
                    key={'circle_' + i}
    ...
    selectedLine = selected !== null ?
      <G><Line
        stroke={'rgba(192,192,192,0.3)'}
        strokeWidth={1}
        x1={xScale (bottomAxisData[selected]) + 10}
        y1={0}
        x2={xScale (bottomAxisData[selected]) + 10}
        y2={yy}
    />
      <G x={xScale (bottomAxisData[1]) + 10} y={ 0 }>
        <Rect
        ...
        <Text
          ...
          textAnchor={ 'middle' }
        >
          { legendText.map((item, index) => {
            return <TSpan x={75 / 2} dy={20} key={index} stroke={ legendColor[index] }> { `${item}: ${data[index][selected].y}` }</TSpan>
          }) }
        </Text>
        </G>
    </G> : null
